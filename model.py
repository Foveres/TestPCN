...
class MSN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(0,self.n_primitives):    # 【K的值0-15】
            rand_grid = Variable(torch.cuda.FloatTensor(x.size(0),2,self.num_points//self.n_primitives)) #rand_grid创建了一个[4,2,512],4个2*512矩阵，值为0
            ...
            # 或者原来的数值。
            y = x.unsqueeze(2).expand(x.size(0),x.size(1), rand_grid.size(2)).contiguous()  # x在第二个轴扩展一个轴[4,1024,512],跳出循环继续下一次循环
        ...
        delta = self.res(xx)
        xx = xx[:, 0:3, :] 
        out2 = (xx + delta).transpose(2,1).contiguous()   # 最终输出
        return out1, out2, loss_mst # out1：torch.Size([4, 8192, 3])；  tout2：orch.Size([4, 8192, 3])